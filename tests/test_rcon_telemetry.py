...
@pytest.mark.asyncio
async def test_request_cancellation_disconnects_without_retry_or_backoff():
    started = asyncio.Event()
    release = asyncio.Event()
    writer = Writer()
    channel = PersistentRconTelemetry("profile", sleep=lambda _delay: pytest.fail("backoff"))
