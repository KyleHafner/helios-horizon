...
from game_control.adapters.crafty import CraftyAdapter, parse_version_text
...
def test_crafty_version_parser_extracts_minecraft_version_from_variables_file() -> None:
    variables = """# Generated by the Forge server image
TYPE=FORGE
VERSION=1.20.1
FORGE_VERSION=47.3.0
SERVER_JAR=forge-1.20.1-47.3.0-server.jar
"""

    assert parse_version_text(variables) == "1.20.1"


def test_crafty_version_parser_extracts_version_from_single_line_tmodloader_text() -> None:
    value = "tModLoader v2026.05.3.0 url=https://github.com/tModLoader/tModLoader"

    assert parse_version_text(value) == "2026.05.3.0"


def test_crafty_version_parser_extracts_version_from_tmodloader_version_file() -> None:
    value = """tModLoader v2026.05.3.0
url=https://github.com/tModLoader/tModLoader/releases/download/v2026.05.3.0/tModLoader.zip
archive_sha256=deadbeef2026.05.3.0
loader=stable
channel=release
build_url=https://example.invalid/v999.999.999.9/build
"""
...
@pytest.mark.parametrize(
    "garbage",
    ["# comments only\nnot=a-version", "not a version\nwith multiple lines", "x" * 4096],
)
def test_crafty_version_parser_returns_bounded_fallback_for_garbage(garbage: str) -> None:
    result = parse_version_text(garbage)
...
@pytest.mark.asyncio
async def test_crafty_observe_sanitizes_multiline_version_payload() -> None:
    class Response:
        content = b'{"data":{"running":true,"version":"VERSION=1.20.1\\nFORGE_VERSION=47.3.0"}}'
    ...
    assert (await adapter.observe(_profile(adapter=AdapterKind.CRAFTY))).installed_version == "1.20.1"
...
@pytest.mark.asyncio
async def test_crafty_observe_parses_stopped_stringified_stats_payload() -> None:
    class Response:
        content = (
            b'{"data":{"running":false,"crashed":false,"online":0,'
            b'"int_ping_results":"False","players":"False",'
            b'"started":"False","version":"False","desc":"False",'
            b'"waiting_start":false}}'
        )
    ...
    assert observation.healthy in {None, False}
    assert observation.started_at is None


@pytest.mark.asyncio
async def test_crafty_observe_parses_running_stringified_stats_payload() -> None:
    class Response:
        content = (
            b'{"data":{"running":true,"crashed":false,"online":3,'
            b'"int_ping_results":"True","players":"False",'
            b'"started":"2026-07-15T02:00:00+00:00",'
            b'"version":"False","desc":"False","waiting_start":false}}'
        )
    ...
    assert observation.players_online == 3


@pytest.mark.asyncio
@pytest.mark.parametrize("ping_results", [{"tcp": True}, [True, True]])
async def test_crafty_observe_keeps_structured_ping_results_working(ping_results) -> None:
    import json

    class Response:
        content = json.dumps(
            {"data": {"running": True, "crashed": False, "int_ping_results": ping_results}}
        ).encode()
    ...
    adapter = CraftyAdapter("https://crafty.invalid", "token", client=Client())
    assert (await adapter.observe(_profile(adapter=AdapterKind.CRAFTY))).healthy is True
...
async def test_systemd_logs_include_aware_time_range_in_argv() -> None:
    ...
    assert seen and seen[0][-6:] == (
        "--since", "2026-07-13 12:00:00.000000 UTC",
        "--until", "2026-07-13 13:00:00.000000 UTC", "-n", "5"
    )
